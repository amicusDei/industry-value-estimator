...
flowchart TD
    ...
    PDATA --> ANCHORS["data/processed/\nmarket_anchors_ai.parquet"]
    ANCHORS --> STAT["src/models/statistical/\n(ARIMA, Prophet)"]
...
| `src/processing/` | Deflate nominal monetary series to 2020 constant USD; interpolate gaps with transparency flagging; apply industry/segment tags; build flat macro indicator matrix for LightGBM features; validate against PROCESSED_SCHEMA |
...
| `config/industries/` | Single source of truth for indicator codes, TRBC sector codes, segment definitions, scope mapping table, and model calibration parameters |
...
- **Analyst-anchored USD market sizes (v1.1):** Market sizes are estimated directly in USD billions using scope-normalized analyst consensus data from 8 firms. The reconciled market anchor series serves as the Y-variable for ARIMA/Prophet. No PCA composite index or value chain multiplier is used. Macro proxy indicators (R&D spend, patents, ICT exports) feed into LightGBM as exogenous features for residual correction.
...
3. `src/processing/features.py` — understand macro indicator matrix construction for LightGBM features
